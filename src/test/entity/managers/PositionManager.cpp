...
TEST(PositionManagerTest, PositionDefaultsToNULL)
{
	...
	EXPECT_EQ(PositionManager::NULL_POS.x, pm.getComponent(e).x);
	EXPECT_EQ(PositionManager::NULL_POS.y, pm.getComponent(e).y);

	EXPECT_EQ(PositionManager::NULL_POS.x, pm.getX(e));
	EXPECT_EQ(PositionManager::NULL_POS.y, pm.getY(e));
}

TEST(PositionManagerTest, SetComponent)
{
	PositionManager pm;
	Entity e = 5;
	PositionComponent pos = {4, 2};

	pm.setComponent(e, pos);

	EXPECT_EQ(pos.x, pm.getComponent(e).x);
	EXPECT_EQ(pos.y, pm.getComponent(e).y);

	EXPECT_EQ(pos.x, pm.getX(e));
	EXPECT_EQ(pos.y, pm.getY(e));
}

TEST(PositionManagerTest, SetComponentByCoordinate)
{
	PositionManager pm;
	Entity e = 5;
	...
	EXPECT_EQ(4, pm.getComponent(e).x);
	EXPECT_EQ(2, pm.getComponent(e).y);
...
TEST(PositionManagerTest, FindEntityByPosition)
{
	...
	pm.setComponent(e1, pos1);
	pm.setComponent(e2, pos2);
